    ...
    try {
      ...
      pollTask(res.task_id, 'progress-' + panelPrefix, 'progress-' + panelPrefix + '-fill', 'progress-' + panelPrefix + '-text', (result) => {
        ...
        if (n > 0) {
          ...
          toast(
            'Загружено 0. На Ozon API новых отзывов/вопросов не найдено. '
            + 'Проверьте: API-ключ именно от выбранного магазина, Premium Plus, права review/list. '
            + 'В ЛК отдельно нужна подписка «Управление отзывами», если видите баннер «Подключить подписку».',
            'info',
          );
        } else {
