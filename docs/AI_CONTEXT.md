...
| Недочёты связок Ozon | `app/core/ozon_link_deficits.py` |
...
### Ozon: запрет на изменение хар-к при выравнивании связок (2026-09-03, уточн. 2026-09-07)

При авто/ИИ/TG-заполнении атрибутов перед склейкой **никогда не менять**:

- ТН ВЭД  
- бренд  
- **габариты упаковки** (длина/ширина/высота) и **вес упаковки** / вес с упаковкой

**Можно:**

- при склейке фасовок: **количество в упаковке** / «единиц в одном товаре» (1/2/3 шт) — `qty_pack`
- **вес товара, г** — из названия/описания/интернета **или** логически: для жидкостей `мл × шт` (1 мл ≈ 1 г)
- при дозаполнении недочётов связки: название цвета, объём мл, единиц, вес товара — модуль `ozon_link_deficits.py`

Порядок источников недочётов: title/description → (опц.) ИИ по этому тексту → web search → ИИ по сниппетам. Фасовки 1/2/3 шт одного товара — units брать из **этой** карточки.

Если конфликт только в запрещённых полях — не править карточки; режим `ozon_link_by_model(category_only=True)`. Для явных фасовок 1/2/3 — `qty_pack=True`.

### Яндекс.Маркет: связки (2026-09-06)

Склейка = одно **«Название группы вариантов»** (`parameterId=200`) у offer_id одной `marketCategoryId`.
API: `yam_link_by_group` / `yam_unlink_cards` / `fetch_yam_catalog`. Отдельного merge нет.
Distinctive-поля Маркет сверяет сам; мы в category_only их не трогаем.
...
- **Сетевые ошибки WB:** ретраить не только HTTP-статусы, но и `RETRYABLE_NETWORK_ERRORS` из `net.py`. Постоянный сбой соединения → `NetworkUnavailableError` (наследник `HttpStatusError`, 504), чтобы обработчики `except HttpStatusError` пометили строку и продолжили, а не оборвали магазин (BUG-00C)
- **Поля документов WB:** имена характеристик матчить в порядке «срок действия → дата регистрации → номер» и по принципу «первое совпадение выигрывает». Имя «Дата **окончания** действия декларации» содержит и «дата», и «декларац» — широкая регулярка регистрации его перехватывает, и дата начала теряется (BUG-00A)
- **Кэш каталога WB** (`packaging_dims_cards`, общий с «Габаритами» и «Характеристиками»): не удалять перед загрузкой — замена идёт одной транзакцией, прошлый снимок хранится в `packaging_dims_cards_prev` и возвращается через `packaging_dims_cache_restore_previous`. В загрузке документов каталог обновляется только по кнопке (`auto_reload_on_miss=False`)
