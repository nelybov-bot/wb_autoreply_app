...
### Ozon: запрет на изменение хар-к при выравнивании связок (2026-09-03)

При авто/ИИ/TG-заполнении атрибутов перед склейкой **никогда не менять**:

- ТН ВЭД  
- бренд  
- вес и габариты (длина/ширина/высота упаковки и вес с упаковкой)

**Можно** ставить при склейке фасовок: **количество в упаковке** / «единиц в одном товаре» (1/2/3 шт) — как в `qty_pack`.

Если конфликт только в запрещённых полях — не править карточки; режим `ozon_link_by_model(category_only=True)` (только категория; qty не обязателен). Для явных фасовок 1/2/3 — `qty_pack=True`. Смысловые поля (тип кожи, аромат…) — можно majority / ИИ / кнопки в Telegram.
