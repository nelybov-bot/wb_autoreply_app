...
## 2026-09-21 — OZON ES: выгрузка деклараций + бренд + ТН ВЭД

- `ozon_client`: list_all сертификатов + `certificate/products/list`
- Скрипт `data/ozon_es_certs_export.py` → Excel (Сводка / Все товары / По декларациям / Без привязки)
- Прогон ES: 5771 товаров, 112 документов, 4016 с привязкой, 1755 без
...
- `_CERT_BATCH_SIZE = 1000` (было 300 по умолчанию) — 500 строк уходят одним `cards/update`
- Каталог и индексы освобождаются сразу после сопоставления артикулов (`del by_vendor, by_nm_id, by_barcode`)
- `packaging_dims._CACHE_TTL_S`: 24 ч → 7 дней; подпись у чекбокса «Обновить каталог с WB» в «Габаритах» поправлена
- T-008 в TASKS: точечная выборка артикулов из SQLite вместо всего каталога — база для возврата параллельности
...
- `_CATALOG_CONCURRENCY = 1` + `catalog_gate`: каталог (≈183 МБ на 12 тыс. карточек) грузится по одному магазину, индексы удаляются сразу после сопоставления — иначе три каталога сразу ≈ 550 МБ при 512 МБ инстанса; отправка остаётся параллельной
...
## 2026-09-20 — Avito: edit только reply + таймер ≤1 мин

- «Изменить»: текст принимается только как reply на карточку
- Задержка отправки в Avito: 15–60 сек
- Новый DEFAULT_PROMPT (цена всегда запрещена, частичный SKIP, продающее поведение)

## 2026-09-20 — Avito AI: цена только если спросили

- Промпт: не писать цену/₽ на вопросы вроде «могу заказать?»
- К клиенту всегда на Вы; от лица компании «мы»
- На голое приветствие: только привет + «чем помочь», без питча доставки

## 2026-09-20 — Avito TG: современный дизайн карточек

- Карточка без emoji-шума: заголовок `Avito · магазин`, buyer/ответ в `blockquote`
- Единый layout для алерта и ИИ-черновика

## 2026-09-20 — Avito AI: живые ответы, без дефисов

- Промпт разговорный, без канцелярита; запрет `-`/`–`/`—` в ответе
- Пост-обработка `_strip_dashes` + style appendix к любому промпту

## 2026-09-20 — Avito TG: без спама (edit одного сообщения)

- Черновик / таймер / результат — `editMessageText` того же алерта, без новых пузырей
- `format_unified_card` + `alert_tg_message_id` из `avito_notify`
