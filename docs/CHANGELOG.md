...
## 2026-06-16 — UI-виджеты (лампа, корова, loading, свечи)

### Frontend
- `fx-widgets.css` / `fx-widgets.js`: лампочка со шнуром (тема), корова при 5xx/сети, loading с точкой, свечи на логине
- Лампочка справа в `/app` — переключение темы без настроек
- Корова: toast снизу слева, cooldown 12 с, не на 4xx
- Прогресс-бары: бегущая точка по тексту (цвет accent)
- Логин: свечи задувают перед запросом auth
